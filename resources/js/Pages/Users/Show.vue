<script>
import { Head, Link, router } from '@inertiajs/vue3'
import Swal from 'sweetalert2'
import { useI18n } from 'vue-i18n'
import { ref } from 'vue'
import Layout from '../../Shared/Layout.vue'

export default {
  components: { Head, Link },
  layout: Layout,
  props: {
    user: Object,
  },
  setup() {
    const currentId = ref('')
    const isVisible = ref(false)

    const toggleVisible = (id) => {
      isVisible.value = !isVisible.value
      currentId.value = id
    }

    const { t } = useI18n({
      useScope: 'global',
      legacy: false,
    })

    const handleDelete = (id) => {
      Swal.fire({
        title: t('notifications.delete.question'),
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: t('notifications.delete.answer'),
        cancelButtonText: t('actions.forms.cancel'),
      }).then((result) => {
        if (result.isConfirmed) {
          Swal.fire(
            t('notifications.delete.info'),
            t('notifications.delete.message'),
            'success',
          )
          router.delete(`/users/${id}/delete`)
        }
      })
    }
    return { handleDelete, t, currentId, isVisible, toggleVisible }
  },
}
</script>

<template>
  <Head title="Detail Pengguna " />
  <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg sm:border border-gray-200 mt-3">
    <table class="table">
      <thead>
        <tr>
          <th class="w-4/8 text-left">
            {{ $t('users.table.name') }}
          </th>
          <th class="w-3/8 text-left">
            {{ $t('users.table.email') }}
          </th>
          <th class="action">
            {{ $t('actions.tables.column') }}
          </th>
        </tr>
      </thead>
      <tbody class="border rounded-lg">
        <tr class="no-content">
          <td> {{ user.name }} </td>
          <td> {{ user.email }} </td>
          <td class="action" @click="toggleVisible(user.id)">
            <div class="text-left">
              <div name="trigger" class="m-auto select-none justify-between cursor-pointer">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.75a.75.75 0 110-1.5.75.75 0 010 1.5zM12 12.75a.75.75 0 110-1.5.75.75 0 010 1.5zM12 18.75a.75.75 0 110-1.5.75.75 0 010 1.5z" />
                </svg>
                <div @click="toggleVisible">
                  <div v-if="currentId === user.id && isVisible">
                    <Link
                      v-if="can('users.edit')" method="get"
                      as="button"
                      :href="`/users/${user.id}/edit`" class="block px-2 py-5 text-sm leading-5 text-green-600 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out" @click="$store.commit('SET_SIDEBAR', {
                        id: 'Edit Pengguna',
                        en: 'Edit User',
                      }) "
                    >
                      {{ $t('actions.forms.edit') }}
                    </Link>
                    <button v-if="can('users.delete')" class="bg-transparent text-green-600 block px-2 py-5 text-sm leading-5 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out" @click="handleDelete(`${user.id}`)">
                      {{ $t('actions.forms.delete') }}
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </td>
        </tr>
        <tr v-if="user.length === 0">
          <td colspan="3" class="text-center">
            {{ $t('users.table.result') }}
          </td>
        </tr>
      </tbody>
    </table>
  </div>
</template>

<style>

</style>
